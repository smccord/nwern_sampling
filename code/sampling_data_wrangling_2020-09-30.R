...
subsample_df <- do.call("rbind", subsample_lpi) %>% do.call("rbind",.)
...
subsample_df <- subsample_df %>% dplyr::mutate(
  VisitID = PrimaryKey, 
  PrimaryKey = paste(PrimaryKey, 
                     pin_drop, 
                     transect_length, 
                     sep = "_"))
...
lines_calcs <- dplyr::left_join(
  bare_soil_lines <- terradactyl::pct_cover_bare_soil(lpi_tall = subsample_df,
                                                      by_line = TRUE, tall = TRUE) %>% 
    # summarize to bare soil "S"
    dplyr::group_by(PrimaryKey, LineKey) %>% dplyr::summarise(BareSoilCover = sum(percent)),
  total_foliar_lines <- terradactyl::pct_cover_total_foliar(lpi_tall = subsample_df,
                                                            by_line = TRUE, tall = FALSE)) %>% 
  dplyr::left_join(
    litter_lines <- terradactyl::pct_cover_between_plant(lpi_tall = subsample_df,
                                                         by_line = TRUE, tall = TRUE) %>% 
      # summarize to one code
      dplyr::group_by(PrimaryKey, LineKey) %>% dplyr::summarise(BetweenPlantCover = sum(percent))
    
  )

# add sampling groups back in
lines_calcs <- lines_calcs %>% dplyr::left_join(subsample_df %>% dplyr::select(PrimaryKey, pin_drop, transect_length, VisitID )) %>% 
  # remove any instances of LineKey = "0"
  subset(LineKey!=0)
# check assumptions
hist(lines_calcs$TotalFoliarCover)
